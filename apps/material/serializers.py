...
class PictureSerializer(serializers.ModelSerializer):
    ...
    def get_image(self, picture):
        if picture.image:
            return "http://{0}/{1}".format(PHOTO_URL, picture.image)
...
class MaterialPostBaseInfoSerializer(serializers.ModelSerializer):
    ...
    def get_front_image(self, article):
        if article.front_image:
            return "http://{0}/{1}".format(PHOTO_URL, article.front_image)
    ...
    class Meta:
        model = PostBaseInfo
        fields = (
            'id', 'title', 'en_title', 'desc', 'en_desc', 'tags', 'like_num', 'comment_num', 'click_num', 'front_image',
            'front_image_type',
            'is_hot',
...
class MaterialSocialSerializer(serializers.ModelSerializer):
    ...
    def get_image(self, social):
        if social.image:
            return "http://{0}/{1}".format(PHOTO_URL, social.image)
